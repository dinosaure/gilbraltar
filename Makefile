...
GLOBAL_CFLAGS=$(MAKECONF_CFLAGS) -I$(MAKECONF_PREFIX)/rpi4-sysroot/include/nolibc/ -include _freestanding/overrides.h
# LIBS used by the OCaml compiler to link executables
GLOBAL_LIBS=-L$(MAKECONF_PREFIX)/rpi4-sysroot/lib/nolibc/ -lnolibc -lopenlibm $(MAKECONF_EXTRA_LIBS)

# NOLIBC
NOLIBC_CFLAGS=$(LOCAL_CFLAGS) -I$(TOP)/openlibm/src -I$(TOP)/openlibm/include
nolibc/libnolibc.a: $(KERNEL)
	$(MAKE) -C nolibc \
	    "CC=$(MAKECONF_CC)" \
...
ocaml/Makefile.config: $(KERNEL) ocaml/Makefile openlibm/libopenlibm.a nolibc/libnolibc.a
	cd ocaml && \
		CC="$(MAKECONF_CC)" \
		OC_CFLAGS="$(OC_CFLAGS)" \
